...
MODEL:          Decision Tree with K-Fold Cross Validation
...
for depth in [1,5,10,15,20,25]:
    ...
    score = np.average(cross_val_score(dt, X, y, cv=kfold, scoring='accuracy'))
    depth_score = np.vstack((depth_score, [depth, score]))
...
depth_score = np.delete(depth_score, 0, 0)

dt = DecisionTreeClassifier(random_state=0, **best_depth)
pred_y = cross_val_predict(dt, X, y, cv=kfold)
...
perf = model_perf_measure(y, pred_y)
print(perf.T)
...
plt.figure(1)
plt.plot(depth_score.T[0],depth_score.T[1])
plt.xlabel('Tree Depths')
plt.xticks([1,5,10,15,20,25])
plt.ylabel('Accuracy')
plt.title('Epoch Error Curve: dt_kfold')
plt.show()
...
cm_0 = calculate_contingency(y, np.asarray(t_0))
cm_25 = calculate_contingency(y, np.asarray(t_25))
cm_5 = calculate_contingency(y, np.asarray(t_5))
cm_75 = calculate_contingency(y, np.asarray(t_75))
cm_1 = calculate_contingency(y, np.asarray(t_1))
...
#model's
fpr, tpr, thresholds = roc_curve(y, pred_y, pos_label=1)
roc_auc = auc(fpr, tpr) 

#threshold 0.0 
fpr_0, tpr_0, thresholds_0 = roc_curve(y, np.asarray(t_0), pos_label=1)
roc_auc_0 = auc(fpr_0, tpr_0) 

#threshold 0.25
fpr_25, tpr_25, thresholds_25 = roc_curve(y, np.asarray(t_25), pos_label=1)
roc_auc_25 = auc(fpr_25, tpr_25)

#threshold 0.5
fpr_5, tpr_5, thresholds_5 = roc_curve(y, np.asarray(t_5), pos_label=1)
roc_auc_5 = auc(fpr_5, tpr_5)

#threshold 0.75 
fpr_75, tpr_75, thresholds_75 = roc_curve(y, np.asarray(t_75), pos_label=1)
roc_auc_75 = auc(fpr_75, tpr_75)

#threshold 1.0
fpr_1, tpr_1, thresholds_1 = roc_curve(y, np.asarray(t_1), pos_label=1)
...
plt.title('ROC Curve: dt_kfold')
plt.legend(loc="lower right")




"""---------------------Bar Graph of Contingency Values-----------------"""
