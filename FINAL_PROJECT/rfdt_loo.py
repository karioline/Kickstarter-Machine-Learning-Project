...
MODEL:                  Random Forest Decision Tree Model with Leave One Out

To use LOO, needed to reduce data set size. So, this model will be using only 1% of the data set.
...
# REDUCING FEATURES FOR LEAVE ONE OUT 
data_reduced = ks_data_all[['main_category', 'usd_goal']]
data_reduced_dummies = pd.get_dummies(data_reduced)
features_reduced = data_reduced_dummies.loc[:, 'usd_goal' : 'main_category_Theater']
X_reduced = features_reduced.values
...
#Reducing data samples used.
#just using X_data and y_data
X_, X_data, y_, y_data = train_test_split(X_reduced, y, random_state=0, test_size=.005)

best_score = 0
best_depth = {}
scores = [0,0] #[num_clfs, depth, acc]
...
for depth in [1,5,10,15]:
    forest = RandomForestClassifier(random_state=0, max_depth=depth)
    acc = np.average(cross_val_score(forest, X_data, y_data,cv=loo,scoring='accuracy'))
...
forest = RandomForestClassifier(**best_depth, n_jobs=-1, random_state=0)
pred_y = cross_val_predict(forest, X_data, y_data, cv=loo)
...
plt.title('Epoch Error Curve: rfdt_loo')
...
rates = np.asarray(([fpr_0[0],fpr_0[0]],  [fpr[1],tpr[1]], [fpr_5[1],tpr_5[1]], [fpr_75[1],tpr_75[1]], [fpr_25[1],tpr_25[1]], [fpr_1[1],tpr_1[1]]))
