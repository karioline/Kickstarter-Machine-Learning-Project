...
MODEL:                  Decision Tree Model with Leave One Out
...
#Reducing the number of samples from 331675 to 3317
#just going to use X_data and y_data
X_, X_data, y_, y_data = train_test_split(X_reduced, y, random_state=0, test_size=.01)
...
for depth in [1,5,10,15]:
    ...
    score = np.average(cross_val_score(dt, X_data, y_data, cv=loo,scoring='accuracy'))
...
pred_y = cross_val_predict(dt, X_data,y_data,cv=loo)
...
plt.title('Epoch Error Curve: dt_loo')


"""-------------------------- Threshold Work -----------------------------"""

dt.fit(X_data,y_data)
u = dt.predict_proba(X_data)
t_0 = []
t_25 = []
t_5 = []
t_75 = []
