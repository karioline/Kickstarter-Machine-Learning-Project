...
MODEL:    Random Forest Decision Tree with K-Fold Cross Validation
...
best_score = 0
best_depth = {}
scores = [0,0] #need this to plot epoch-like error curve
...
for depth in [10,15,20,25,30,35]:
    ...
    acc = np.average(cross_val_score(forest, X, y, scoring="accuracy", cv=kfold))
...
pred_y = cross_val_predict(forest, X, y, cv=kfold)
...
plt.ylabel('Accuracy')
plt.title('Epoch Error Curve: rfdt_kfold')
plt.show()
...
plt.title('ROC Curve: rfdt_kfold')
